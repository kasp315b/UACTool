...
public class UACTool {
	...
	// {{ All in One Methods
	public UACTool setSettings(File executable, String arguments, boolean showWindow, boolean useIntermediate, boolean showPopup) {
		if(hasPrepared) {
	...
	public UACTool setSettings(String executablepath, String arguments, boolean showWindow, boolean useIntermediate, boolean showPopup) {
	...
	public void prepare() throws IOException {
		if(useIntermediate) {
			...
			if(getFiletype(executable.getName()).equals(".jar")) System.out.println("It is recommended to use an intermediate when launching jar files.");
	...
	public void execute() throws IOException {
		...
		if(showPopup) {
			...
			JOptionPane.showMessageDialog(null, parsedMessage, "Administrator Privilege Request", JOptionPane.INFORMATION_MESSAGE);
		}
	...
	public static boolean runAsAdmin(File executable, String arguments, boolean showWindow, boolean useIntermediate, boolean showPopup) {
	...
	public static boolean runAsAdmin(String executablepath, String arguments, boolean showWindow, boolean useIntermediate, boolean showPopup) {
		return runAsAdmin(new File(executablepath), arguments, showWindow, useIntermediate, showPopup);
	}
	
	public static boolean runAsAdmin(String executablepath, String arguments, boolean showWindow, boolean useIntermediate) {
	...
	private static String getVBSScript(String applicationpath, String arguments, boolean visible) {
		String vbs =
				...
				"UAC.ShellExecute "+addQuotes(applicationpath)+", "+addQuotes(arguments)+", strFolder, \"runas\", "+(visible ? "1" : "0")+"\r\n";
		return vbs;
	}
	...
	private static String getRandomFilename(int length, String extension) {
		...
		for(int i = 0; i < length; i++) sb.append(charset[(int)(Math.floor(Math.random() * charset.length))]);
		sb.append(extension);
		return sb.toString();
	}
	
	private static String addQuotes(String toQuotify) {
		if(toQuotify == null) return null;
		if(toQuotify.equals("")) return "\"\"";
		return (toQuotify.charAt(0) == '"' ? "" : '"') + toQuotify + (toQuotify.charAt(toQuotify.length()-1) == '"' ? "" : '"');
	}
